    ...
    $[_PLUGIN_].prototype = {
        ...
        settingDOM : function (){
            ...
            this.resourceSrcArr = this.container.find('img').map(function(i,e){
                var href = $(e).data("href") ? $(e).data("href") : 'javascript:void(0);';
                var target = $(e).data("href") ? "_blank" : "_self";
                return {src: e.src, href: href, target: target};
            });
            var ulDOM = this.container.html('<ul class="dnSlide-list"></ul>').find('.dnSlide-list');

            jQuery.each(this.resourceSrcArr , function(i,e){
                ulDOM.append('<li class="dnSlide-item"><a  target="' + _this_.resourceSrcArr[i]["target"] + '" href="'+_this_.resourceSrcArr[i]["href"]+'"><img class="slide-img" src="'+_this_.resourceSrcArr[i]["src"]+'" width="100%"></a></li>');
            });
